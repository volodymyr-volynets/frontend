...
class Base implements Interface2\Base
{
    ...
    public static function setMode(bool $status)
    {
        ...
        if (self::$is_email) {
            ...
            \Numbers\Frontend\HTML\Renderers\DOMParser\Base::load('email', Ob::clean());
        }
    }
    ...
    protected static function generateAttributes($options, $tag = null)
    {
        ...
        foreach ($options as $k => $v) {
            ...
            if (!HTML5::isValidHTML5Attribute($k, $tag)) {
                continue;
            }
            if (in_array($k, HTML5::$strip_tags)) {
                $v = strip_tags($v . '');
            }
            if (is_array($v)) {
                if (array_values($v) !== $v) {
    ...
    private static function generateSelectOptions($data, $value, $options = [])
    {
        ...
        foreach ($data as $k => $v) {
            ...
            } elseif (!is_array($value) && ($value . '') === $k) {
                $selected = ' selected="selected" ';
            }
            // we need to skip certain options
            if (!empty($options['readonly']) && !empty($options['filter_only_selected_options_if_readonly']) && empty($selected)) {
                continue;
            }
            $temp = '';
            if (empty($v['disabled'])) {
    ...
    public static function textarea(array $options = []): string
    {
        ...
        return '<textarea ' . self::generateAttributes($options, 'textarea') . '>' . htmlspecialchars($value, ENT_NOQUOTES | ENT_SUBSTITUTE | ENT_HTML401) . '</textarea>';
    }
    ...
    public static function table(array $options = []): string
    {
        ...
        $trim_columns = $options['trim_columns'] ?? false;
        if ($trim_columns === true) {
            $trim_columns = 50;
        }
        ...
        if (!empty($options['show_row_number'])) {
            $header = array_merge_hard(['__number' => '#'], $header);
        }
        $result = [];
        $first_column = null;
        // header first
        ...
        foreach ($rows as $k => $v) {
            ...
            foreach ($header as $k2 => $v2) {
                ...
                if (is_array($v[$k2])) {
                    ...
                    // trim plain columns
                    if ($trim_columns && !is_html($temp_value)) {
                        $temp_value = (new \String2($temp_value))->truncate($trim_columns)->toString();
                    }
                    ...
                    // trim plain columns
                    if ($trim_columns && !is_html($v[$k2])) {
                        $v[$k2] = (new \String2($v[$k2]))->truncate($trim_columns)->toString();
                    }
                    $temp2 .= '<td>' . $v[$k2] . '</td>';
                }
            }
        ...
        // if we have no data
        if (!empty($options['show_zero_rows']) && empty($rows)) {
            $result[] = '<tr><td colspan="' . count($header) . '">' . loc(Error::NO_ROWS_FOUND) . '</td></tr>';
        }
        // todo: add footer
