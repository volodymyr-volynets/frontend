        ...
        if (preg_match_all('/"([^"]+?\.css)"/', $html, $matches)) {
            ...
            foreach ($matches[1] as $v) {
                if (strpos($v, '/numbers/') === 0 && \Application::get('environment') == 'development') {
                    $css .= Media::serveMediaIfExists($v, \Application::get(['application', 'path']), true);
                } elseif ($v[0] == '/') {
    ...
    public static function getStyle(string $link, string $tag, string $id, string $class): string
    {
        ...
        foreach ($result as $k => $v) {
            if ($link == 'email') {
                ...
                if (strpos($k, 'background') !== false) {
                    $found = true;
                }
                if (strpos($k, 'border') !== false) {
                    $found = true;
                }
                if (strpos($k, 'padding') !== false) {
                    $found = true;
                }
                if (strpos($k, 'display') !== false && strpos($v, 'none') !== false) {
                    $found = true;
                }
                if ($found) {
                    $temp[] = $k . ': ' . $v . ';';
