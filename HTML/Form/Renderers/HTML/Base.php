...
class Base
{
    ...
    public function render(\Object\Form\Base & $object): string
    {
        ...
        if (!empty($this->object->options['actions']['new']) && \Application::$controller->can('Record_New', 'Edit')) {
            if ($mvc['action'] == 'Edit') {
                $onclick = 'return confirm(\'' . strip_tags(i18n(null, Messages::CONFIRM_BLANK)) . '\');';
            } else {
                $onclick = '';
            }
            $params = [];
        ...
        // other actions
        foreach ($this->object->options['actions'] ?? [] as $k => $v) {
            if (in_array($k, ['refresh', 'new', 'back', 'activate'])) {
                continue;
            }
            ...
            if (!$v['flag_child']) {
                if ($v['type'] == 'fields' || $v['type'] == 'details' || $v['type'] == 'trees') {
                    ...
                    if ($k == $this->object::LIST_CONTAINER || $k == $this->object::LIST_LINE_CONTAINER) {
                        ...
                        } elseif ($preview == 2 && $k == $this->object::LIST_LINE_CONTAINER) {
                            $temp = $this->renderListContainer($k);
                            if ($temp['success']) {
                                $result[$k] = $temp['data'];
                            }
                ...
                } elseif ($v['type'] == 'tabs') { // tabs
                    $this->renderTabs($k, $v, $result);
                } elseif ($v['type'] == 'panels') { // pannels
                    $this->renderPanels($k, $v, $result);
                } elseif ($v['type'] == 'modal') { // modal windows
                    // reset tabs
                    $this->object->current_tab = [];
                    // render container
                    $temp = $this->renderContainer($k);
        ...
        if (!$this->object->hasErrors()) {
            ...
            } elseif (!empty($this->object->options['on_success_refresh_parent']) && $submitted && !$this->object->refresh && !$this->object->blank) {
                $js .= "Numbers.Modal.hide('form_subform_{$this->object->form_link}_form');\n";
                $js .= "$('#form_{$this->object->options['parent_form_link']}_form').submit();\n";
            }
        }
...
TTT
        ...
        if (isset($this->object->options['segment'])) {
            ...
            if (isset($temp['header']['title'])) {
                if (isset($temp['header']['loc'])) {
                    $temp['header']['title'] = loc($temp['header']['loc'], $temp['header']['title']);
                } else {
                    $temp['header']['title'] = i18n(null, $temp['header']['title']);
                }
            }
            if (isset($temp['footer']['title'])) {
                if (isset($temp['footer']['loc'])) {
                    $temp['footer']['title'] = loc($temp['footer']['loc'], $temp['footer']['title']);
                } else {
    ...
    private function renderTabs($k, $v, & $result)
    {
        ...
        foreach ($v['rows'] as $k2 => $v2) {
            ...
            } elseif (!empty($this->object->wrapper_methods['overrideTabs']['main'])) { // handling override_tabs method
        ...
        $class = ['form-tabs'];
        if (!empty($v['options']['class'])) {
            $class[] = $v['options']['class'];
        }
        ...
        foreach ($v['rows'] as $k2 => $v2) {
            ...
            if (!empty($temp_panels)) {
                ...
                if (empty($v2['options']['skip_header'])) {
                    if (isset($v2['options']['loc'])) {
                        $header = [
                            'title' => loc($v2['options']['loc'], $v2['options']['label_name']),
    ...
    private function renderListContainerDefault(& $data, & $options)
    {
        ...
        if ($data['num_rows'] == 0) {
            return \HTML::message(['type' => 'warning', 'options' => [i18n(null, Messages::NO_ROWS_FOUND)]]);
        }
        $table = [
            'width' => '100%',
            'options' => []
        ...
        if (empty($data['preview'])) {
            ...
            foreach ($data['rows'] as $k0 => $v0) {
                ...
                if (!empty($this->object->misc_settings['subforms']['url_delete'])) {
                    ...
                    $temp_collection_link = $this->object->options['collection_link'] ?? '';
                    $temp_collection_screen_link = $this->object->options['collection_screen_link'] ?? '';
                    $value = \HTML::a(['href' => 'javascript:void(0);', 'onclick' => "if (confirm('" . strip_tags(i18n(null, Messages::CONFIRM_DELETE)) . "')) { Numbers.Form.openSubformWindow('{$temp_collection_link}', '{$temp_collection_screen_link}', '{$this->object->form_link}', '{$this->object->misc_settings['subforms']['url_delete']['subform_link']}', {$params}, {__submit_delete: true, __hide_popup_window: true}); }", 'value' => '<i class="far fa-trash-alt"></i>']);
        ...
        } elseif ($data['preview'] == 1) { // preview 1 multiline grid
            // generate rows
            $row_number_final = $data['offset'] + 1;
            $cached_options = [];
            foreach ($data['rows'] as $k0 => $v0) {
        ...
        } elseif ($data['preview'] == 2) { // single line grid
    ...
    public function renderContainerTypeDetailsRows($rows, $values, $options = [])
    {
            ...
            foreach ($rows as $k => $v) {
                ...
                foreach ($groupped as $k2 => $v2) {
                    ...
                    foreach ($v2 as $k3 => $v3) {
                        ...
                        $temp_values = null;
                        $temp_neighboring_values = [];
                        $data['options'][$k][$k2][$k3] = [
                            'label' => $this->renderElementName($first, $temp_values, $temp_neighboring_values),
                            'options' => $v3['options'],
        ...
        do {
            ...
            if (!empty($options['details_max_rows']) && $row_number > $options['details_max_rows']) {
                break;
            }
            // render
            $data = [
            ...
            if ($processing_values) {
                ...
                if (!empty($options['details_unique_select_field'])) {
                    ...
                    $options_array_processed = Common::processOptions($column['options']['options_model'], $this->object, $column['options']['options_params'], null, $skip_values, $column['options']['options_options'] ?? []);
                    ...
                    if ($first_key == $this->object::SEPARATOR_HORIZONTAL) {
                        ...
                        foreach ($v2 as $k3 => $v3) {
                            ...
                            if ($options['details_rendering_type'] == 'grid_with_label') {
                                $label = $this->renderElementName($first, $v0[$k3] ?? null, $v0);
                            }
                            // we need to pass proper options
                            if (isset($__readonly)) {
            ...
            if ($__readonly || $__required) {
                $link = '';
            } elseif (empty($options['details_cannot_delete']) && empty($this->object->misc_settings['global']['readonly']) && empty($this->object->misc_settings['acl_subresource_locks'][$options['container_link']]['no_delete'])) {
                $link = \HTML::a(['href' => 'javascript:void(0);', 'value' => '<i class="far fa-trash-alt"></i>', 'onclick' => "if (confirm('" . strip_tags(i18n(null, Messages::CONFIRM_DELETE)) . "')) { Numbers.Form.detailsDeleteRow('form_{$this->object->form_link}_form', '{$row_id}'); } return false;"]);
            } else {
                $link = '';
        ...
        if (empty($options['details_new_rows']) && empty($values) && isset($options['details_empty_warning_message'])) {
            if (empty($options['details_empty_warning_message']) || $options['details_empty_warning_message'] === true) {
                $message = \HTML::message(['type' => 'warning', 'options' => [Messages::NO_ROWS_FOUND]]);
            } else {
    ...
    public function renderRowGrid($rows, $options = [])
    {
        ...
        foreach ($rows as $k => $v) {
            ...
            // processing buttons
            if (in_array($v['key'], [$this->object::BUTTONS, $this->object::TRANSACTION_BUTTONS, $this->object::WIDE_BUTTONS])) {
                $buttons = [
                    'left' => [],
                    'center' => [],
                    'right' => [],
                ...
                if ($first_key == $this->object::SEPARATOR_HORIZONTAL) {
                    ...
                    foreach ($v2 as $k3 => $v3) {
                        ...
                        if ($v['key'] === $this->object::HIDDEN && !\Application::get('flag.numbers.frontend.html.form.show_field_settings')) {
                            $v3['options']['row_class'] = ($v3['options']['row_class'] ?? '') . ' grid_row_hidden';
                            $hidden = true;
                        } elseif ($v['key'] === $this->object::HIDDEN) {
                        ...
                        if (!$hidden) {
                            ...
                            $this->object->tabindex++;
                        } elseif ($hidden && \HTML::getMode()) { // we do not render hidden fields in email
                            continue;
                        }
                ...
                if ($first_key == $this->object::SEPARATOR_HORIZONTAL) {
                    ...
                    if (!empty($first['options']['label_name'])) {
                        ...
                        $inner_data['options'][$k][$k2] = [
                            'value' => $this->renderElementName($first, $temp_value, $temp_neighboring_values),
                            'nowrap' => true,
     ...
     * @return string
     */
    public function renderElementName($options, $value = null, & $neighbouring_values = [])
    {
        if ((isset($options['options']['label_name']) && ($options['options']['label_name'] . '') != '') || isset($options['options']['label_i18n']) || isset($options['options']['loc'])) {
            ...
            if (!empty($options['options']['required'])) {
                ...
                } elseif ($options['options']['required'] == 'c') {
    ...
    public function renderElementValue(& $options, $value = null, & $neighbouring_values = [])
    {
        ...
        // we might set row_hidden in processor
        if (!empty($options['options']['row_hidden'])) {
            $options['options']['row_class'] = ($options['options']['row_class'] ?? '') . ' grid_row_hidden';
        }
        $result_options = $options['options'];
        // loc_options
        if (isset($result_options['loc_options'])) {
            $loc_options = is_array($result_options['loc_options']) ? $result_options['loc_options'] : [$result_options['loc_options']];
            ...
            $loc_options = $result_options;
            $loc_options['value'] = $result_options['loc_options'];
            $loc_options['static'] = false;
            unset($loc_options['loc_options']);
            if (!is_array($result_options['loc_options'])) {
                $result_options['loc_options'] = [$result_options['loc_options']];
            }
            $temp = [];
            foreach ($result_options['loc_options'] as $k => $v) {
                if (is_numeric($k)) {
                    $k = $v;
                    $v = $this->object->form_parent->loc[$k];
                }
                $loc_options['value'] = loc($k, $v);
                $temp[] = $field_method_object->{$method[1]}($loc_options);
            }
            if (isset($value)) {
                if (!is_array($value)) {
                    $value = [$value];
                }
                $temp[] = \HTML::ul(['options' => $value]);
            }
            return implode('', $temp);
        }
        ...
        if (!empty($result_options['options_model'])) {
            ...
            if (strpos($result_options['method'] ?? '', 'autocomplete') === false) {
                ...
                $options_array_processed = Common::processOptions($result_options['options_model'], $this->object, $result_options['options_params'], $value, $skip_values, $result_options['options_options']);
            ...
            case 'field':
                ...
                if (in_array($element_method, ['\HTML::a', '\HTML::submit', '\HTML::button', '\HTML::button2', '\HTML::separator'])) {
                    ...
                    if (in_array($element_method, ['\HTML::submit', '\HTML::button', '\HTML::button2'])) {
                        ...
                        } elseif (empty($result_options['onclick'])) {
                ...
                } elseif (in_array($element_method, ['\HTML::div', '\HTML::span', '\HTML::b', '\HTML::pre'])) {
                    if (!empty($value)) {
                        $result_options['value'] = $value;
                    }
                    // format
                    ...
                    if ($element_type === 'json' && isset($value)) {
                        $json = (new \Json2($value));
                        if ($json->isValid()) {
                            $value = $json->toJSON($result_options['json_nice'] ?? false);
                        }
                    }
                    $result_options['value'] = $value;
                    ...
                    if (!empty($result_options['persistent']) && $this->object->values_loaded) {
                        ...
                        } elseif (count($result_options['values_key']) == 1) { // parent record
                            $result_options['readonly'] = true;
                        } elseif (empty($result_options['__new_row'])) { // details
                ...
                if (in_array($element_method, ['\HTML::select', '\HTML::multiselect']) && !array_key_exists('placeholder', $result_options)) {
                    $result_options['placeholder'] = strip_tags(i18n(null, Messages::PLEASE_CHOOSE));
                }
                // events
                foreach (HTML5::$events as $e) {
                    if (!empty($result_options['readonly'])) { // important - readonly emenets cannot have events
                        unset($result_options[$e]);
                    } elseif (!empty($result_options[$e])) {
