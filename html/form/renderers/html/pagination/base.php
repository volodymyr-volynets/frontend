...
class numbers_frontend_html_form_renderers_html_pagination_base {
	...
	public function render(& $options) {
		...
		if ($current_page >= 1) {
			$navigation[]= html::button2(['value' => i18n(null, 'First'), 'onclick' => "numbers.form.set_value(this.form, '__offset', 0); numbers.form.trigger_submit(this.form);"]);
		}
		if ($current_page >= 2) {
			$previous = (($current_page - 1) * $options['limit']);
			$navigation[]= html::button2(['value' => i18n(null, 'Previous'), 'onclick' => "numbers.form.set_value(this.form, '__offset', {$previous}); numbers.form.trigger_submit(this.form);"]);
		}
		...
		if ($options['num_rows']) {
			...
			$previous = (($current_page - 1) * $options['limit']);
			$navigation2.= '<div style="width: 100px; display: inline-block;">' . html::select(['id' => 'pages_' . $type, 'options' => $temp, 'value' => $options['offset'], 'no_choose' => true, 'onchange' => "numbers.form.set_value(this.form, '__offset', this.value); numbers.form.trigger_submit(this.form);"]) . '</div>';
		...
		if ($flag_next_row_exists) {
			...
			$navigation[]= html::button2(['value' => i18n(null, 'Next'), 'onclick' => "numbers.form.set_value(this.form, '__offset', {$next}); numbers.form.trigger_submit(this.form);"]);
		}
		if ($flag_last_row_exists) {
			$last = (($pages - 1) * $options['limit']);
			$navigation[]= html::button2(['value' => i18n(null, 'Last'), 'onclick' => "numbers.form.set_value(this.form, '__offset', {$last}); numbers.form.trigger_submit(this.form);"]);
		}
		// generating grid
		$grid = [
			'options' => [
