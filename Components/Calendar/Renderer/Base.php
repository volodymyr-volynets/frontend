...
use Helper\Date;
use Numbers\Framework\Helper\Model\Date\WeekDays2;
...
class Base
{
    ...
    private function renderMonth(array $data, array $holidays, string $start_date, array $options = []): string
    {
        ...
        foreach ($holidays as $k => $v) {
            ...
            $v['slot_name'] = $name;
            $v['slot_color'] = Colors::colorFromString($name);
            $v['slot_text_color'] = Colors::determineTextColor($v['slot_color']);
            $data_arranged['holidays'][$v['date']][$k] = $v;
        }
        foreach ($data as $k => $v) {
            $date1a = new \DateTime(\Format::readDate($v['work_starts'], 'datetime'));
            $date2a = new \DateTime(\Format::readDate($v['work_ends'] ?? Date::addInterval($v['work_starts'], '+1 hour'), 'datetime'));
            ...
            if ($date1a->format('Y-m-d') != $date2a->format('Y-m-d')) {
                ...
                while ($date1a->format('Y-m-d') != $date2a->format('Y-m-d')) {
                    ...
                    $v['slot_color'] = Colors::colorFromString($v['name'] . '::' . $v['hash_name']);
                    $v['slot_text_color'] = Colors::determineTextColor($v['slot_color']);
        ...
        for ($i = 1; $i <= 6; $i++) {
            ...
            if ($date1->format('Y-m-d') > $next_month) {
                break;
            }
        }
        $result .= '</table>';
    ...
    private function renderWeek(array $data, array $holidays, string $start_date, array $options = []): string
    {
        ...
        foreach ($data as $k => $v) {
            ...
            $date2 = new \DateTime(\Format::readDate($v['work_ends'] ?? Date::addInterval($v['work_starts'], '+1 hour'), 'datetime'));
            // multi days intervals
            ...
            foreach ($week_days as $k => $v) {
                ...
                if (!empty($data_arranged['single_day'][$k][$i])) {
                    ...
                    foreach ($data_arranged['single_day'][$k][$i] as $k2 => $v2) {
                        ...
                        $date2 = new \DateTime(\Format::readDate($v2['work_ends'] ?? Date::addInterval($v2['work_starts'], '+1 hour'), 'datetime'));
                        ...
                        $zindex++;
                        // push
                        if (!isset($data_single_day_lock[$k])) {
                            $data_single_day_lock[$k] = [];
                        }
